...
from model import TransformerUNet  # <-- Make sure your updated model is correct
...
def estimate_pos_weight(paths, limit=200):
    pos = neg = 0
...
class WetlandTileDataset(Dataset):
    def __init__(self, samples, band_means, band_stds, patch_size, stride, augment=False):
        ...
        ], additional_targets={"masks0": "mask", "masks1": "mask"}) if augment else None
...
def main():
    ...
    if os.path.exists(BEST_MODEL_PATH):
        ...
        best_iou = ckpt.get("val_loss", -1.0)
